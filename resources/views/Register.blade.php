                ...
                <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow border border-[#E2CEB1] rounded-lg bg-white">
                    ...
                    <div class="flex-auto px-4 lg:px-10 py-10 pt-0">
                        <form action="{{ route('register.post') }}" method="POST">
                            ...
                            <!-- Role Selection -->
                            <div class="relative w-full mb-3">
                                <label class="block uppercase text-[#553827] text-xs font-bold mb-2">
                                    Account Type
                                </label>
                                <div class="flex gap-4">
                                    <div class="flex items-center">
                                        <input type="radio" name="role_id" id="role_user" value="2" checked
                                            class="mr-2 focus:ring-2 focus:ring-[#cfad7d]" {{ old('role_id') == '2' ? 'checked' : '' }}>
                                        <label for="role_user" class="text-gray-700">Regular User</label>
                                    </div>
                                    <div class="flex items-center">
                                        <input type="radio" name="role_id" id="role_vendor" value="3"
                                            class="mr-2 focus:ring-2 focus:ring-[#cfad7d]" {{ old('role_id') == '3' ? 'checked' : '' }}>
                                        <label for="role_vendor" class="text-gray-700">Vendor</label>
                                    </div>
                                </div>
                                @error('role_id')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
