    ...
    <div class="min-h-screen bg-[#FFF8E6] flex flex-col md:flex-row font-sans">
        ...
        <div class="relative h-[40vh] bg-gray-100 overflow-hidden md:h-screen md:w-[55%]">
            <div class="relative overflow-hidden w-full h-full">
                ...
                @foreach ($images as $index => $image)
                    <input class="carousel-open absolute opacity-0 invisible" type="radio" id="carousel-{{ $index + 1 }}" name="carousel"
                        aria-hidden="true" {{ $index === 0 ? 'checked="checked"' : '' }}>
                    <div class="carousel-item absolute opacity-0 w-full h-full transition-opacity duration-500 ease-out {{ $index === 0 ? '!opacity-100' : '' }}">
                        <img src="{{ asset($image) }}" alt="Food image {{ $index + 1 }}" class="w-full h-full object-cover object-center">
                    </div>
                @endforeach
        ...
        <div class="p-8 md:p-12 md:w-[45%] md:overflow-y-auto">
            ...
            6.572.955-4.756 4.635 1.123 6.545z" />
                ...
                <!-- Rating number -->
                <span class="ml-2 text-sm text-gray-700 font-medium">4.0</span>
                <span class="ml-2 text-sm text-gray-600 font-medium">(130 reviews)</span>
            </div>
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <div class="inline-block px-2 py-1 rounded-full text-xs font-medium {{ \Carbon\Carbon::parse($menuDay->foodDate)->isToday() ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
            ...
            <div class="flex flex-col gap-4">
                <form action="{{ route('cart.add') }}" method="POST">
                    ...
                    <button type="submit" class="w-full flex justify-center items-center bg-[#E5CBA6] hover:bg-[#d6b88d] text-white font-medium py-3 px-4 rounded-lg transition-all duration-300">
                ...
                <a href="/home" class="w-full flex justify-center items-center bg-white hover:bg-gray-100 text-gray-600 font-medium py-3 px-4 border border-gray-300 rounded-lg transition-all duration-300 text-center">
                    Back to menu
                </a>
            </div>
        </div>
